...
local_wd <- file.path(here(), "dipper_js")
setwd(local_wd)
...
#' -----------------------------------------------------------------------------
source("js_nimble_model_f.R")

js_model <- nimbleModel(
  code = js_code,
  constants = list(K=ncol(x), nobs=nrow(x)),
  data = list(x = x, n = nrow(x)),
  inits = list(f=1, p=rep(0.5,7), phi = rep(0.5,6), lambda=100)
)

c_js_model <- compileNimble(js_model)
js_mcmc <- buildMCMC(js_model, monitors=c("beta","phi","p","lambda","nu","pstar","Nsuper","Nu","Bu","Su","alive", "Nd", "N","Np","f"))
...
summary(mcmc(samples_list$Np))
summary(mcmc(samples_list$f))

Ndf <- data.frame(year = 1:ncol(y), est=colMeans(samples_list$N), hpd = HPDinterval(mcmc(samples_list$N)))
