...
js_model <- nimbleModel(
  code = js_code,
  constants = list(K=K, nobs=nrow(x)),
...
summary(samples_list$f)
HPDinterval(samples_list$rho)

plot(samples_list$rho)
...
summary(samples_list$mu_f)
HPDinterval(samples_list$mu_f)

summary(samples_list$sig_f)
HPDinterval(samples_list$sig_f)
