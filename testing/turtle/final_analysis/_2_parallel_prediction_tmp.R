...
load("honu_tmp_mcmc_sample.RData")
...
Ndf <- data.frame(
  ...
  mutate(
    ...
    aaa = ifelse(year<1983, 0.33, 1)
  )
ggplot(Ndf) +
  # geom_point(aes(x=year, y=est), alpha=Ndf$aaa, size=5, pch='-') +
  geom_errorbar(aes(x=year, ymin=hpd.lower, ymax=hpd.upper), alpha=Ndf$aaa, width=0.2) +
  geom_path(aes(x=year, y=est), alpha=Ndf$aaa) +
