...
js_code <- nimbleCode({
  ...
  for(t in 1:(K-1)){
    log(theta[t]) <- beta_theta_int + beta_theta*temp[t] #inprod(B[t,1:m], beta_theta[1:m])
    }
  ...
  for(j in 1:mp){ beta_p[j] ~ dnorm(0, tau=prec_p) }
  # beta_p ~ dnorm(0,sd=sig_p)
  for(t in 1:K){
    alpha[t] ~ dbeta(5,1)
    p[t,2] <- surv[t]*expit(beta_p_int + inprod(X[t,1:mp],beta_p[1:mp]))
