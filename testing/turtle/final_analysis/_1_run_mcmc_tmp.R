...
temp <- read_csv("raw/winter_temp_anomolies.csv", skip=3) %>% filter(Year<2016)
...
sm <- smoothCon(s(total_days, bs='ts'), data=effort, diagonal.penalty = T)[[1]]
X <- sm$X

source("honu_model_tmp.R")
...
js_model <- nimbleModel(
  ...
  inits = list(
    ...
    beta_p_int=0, beta_p = rep(0,ncol(X)), sig_p=2,
    alpha=rep(5/6,K),
    lambda=2*n
  )
)
mcmcConf <- configureMCMC(js_model,
                          monitors=c(
                            ...
                            "p", "alpha", "beta_p_int", "beta_p", "sig_p",
...
save(samples_list, honu_ch, ch_collapsed, surv, file="honu_tmp_mcmc_sample.RData")
