...
predict_abundance <- nimbleFunction(
  run = function(xi=double(2), phi = double(1), mu=double(2), p=double(3),
    ...
    for(j in 1:iter){
      ...
      for(t in 1:(K-1)){
        zeta <- make_dt_pois(nd, mu[j,t], shift=1)
        Gamma[,,t] <- make_honu_Gamma(xi_tilde[t+1], phi[j], p[j,t+1,], alpha=0, zeta)
