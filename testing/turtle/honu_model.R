...
js_code <- nimbleCode({
  ...
  # pi
  pi[1:n_states] <- make_dt_pi(xi_tilde[1],nd)

  # Gamma
  for(t in 1:(K-1)){
    Gamma[1:n_states, 1:n_states, t] <- make_dt_Gamma(xi_tilde[t+1], phi[t], zeta[1:nd])
  }
  ...
  for(t in 1:K){
    Pmats[1:n_states,1:2,t] <- make_dt_Pmats(p[t],nd)
  }
